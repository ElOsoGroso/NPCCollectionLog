...
public class WikiScraper {
	...
	public static CompletableFuture<ArrayList<WikiItem>> getDropsByMonster(String monsterName, int monsterId, ArrayList<Item> itemIdListIn) {
		CompletableFuture<ArrayList<WikiItem>> future = new CompletableFuture<>();
		...
		requestAsync(url).whenCompleteAsync((responseHTML, ex) -> {
			...
			List<WikiItem> itemList = new ArrayList<>();
			...
			ArrayList<WikiItem> result = (ArrayList<WikiItem>) itemList.stream().filter(distinctByKey(WikiItem::getName)).collect(Collectors.toList());
			future.complete(result);
		});
